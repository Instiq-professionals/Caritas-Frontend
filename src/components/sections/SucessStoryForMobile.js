        ...
        {props.tutorialSteps.map((step, index) => (
          <div key={step._id}>
            {Math.abs(activeStep - index) <= 2 ? (
              <>
              <img className={classes.img} src={baseUrl + step.pictures} alt={step.cause_title} />
              <div className={classes.desc}>
              <span className={classes.category}>{step.category}</span>
              <h4 style={{ color: Colors.appRed, width: "70%", textAlign:'center' }}>
                 {step.cause_title}
              </h4>
              <p style={{ fontSize: 10 }}>{step.testimonial.substr(0, 400)} </p>
                </div>
                <div
                 onClick={() => (window.location = `/story/${step.cause_id}`)}
